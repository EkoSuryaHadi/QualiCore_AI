...
def create_access_token(user: User, session_id: str | None = None) -> str:
    ...
    sig = hmac.new(
        settings.jwt_secret.encode(), signing.encode(), hashlib.sha256
    ).digest()
...
def get_current_user(
    ...
    # New tokens are bound to a server-side session. Legacy tokens without sid
    # remain valid temporarily for a safe production transition.
    sid = payload.get("sid")
    if sid:
