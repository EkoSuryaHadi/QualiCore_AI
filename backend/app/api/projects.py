...
def get_project(db,id,org,user=None):
 p=db.scalar(select(Project).where(Project.id==id,Project.organization_id==org));
 if not p:raise HTTPException(404,'Project not found')
 if user is not None and not can_access_project(db,user,id):raise HTTPException(403,'Project access not assigned')
 return p
...
def list_projects(db:Session=Depends(get_db),user:User=Depends(get_current_user)):
 ...
 if allowed is not None:q=q.where(Project.id.in_(allowed)) if allowed else q.where(False)
...
def detail(id:str,db:Session=Depends(get_db),user:User=Depends(get_current_user)):return get_project(db,id,user.organization_id,user)
...
def workspace(id:str,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
 p=get_project(db,id,user.organization_id,user);scalar=lambda q:db.scalar(q) or 0
 ...
 overdue_punch=scalar(select(func.count()).select_from(PunchItem).where(PunchItem.project_id==id,PunchItem.status!=WorkflowStatus.CLOSED,PunchItem.due_date.is_not(None),PunchItem.due_date<date.today()))
 return dict(project=p,quality_score=score,total_inspections=inspections,failed_inspections=failed,open_ncr=scalar(select(func.count()).select_from(NCR).where(NCR.project_id==id,NCR.status!=WorkflowStatus.CLOSED)),critical_ncr=scalar(select(func.count()).select_from(NCR).where(NCR.project_id==id,NCR.status!=WorkflowStatus.CLOSED,NCR.severity.in_([Severity.HIGH,Severity.CRITICAL]))),open_punch=scalar(select(func.count()).select_from(PunchItem).where(PunchItem.project_id==id,PunchItem.status!=WorkflowStatus.CLOSED)),overdue_punch=overdue_punch,evidence_count=scalar(select(func.count()).select_from(Evidence).where(Evidence.project_id==id)),open_documents=scalar(select(func.count()).select_from(Document).where(Document.project_id==id,Document.status!=DocumentStatus.APPROVED)),high_risks=scalar(select(func.count()).select_from(Risk).where(Risk.project_id==id,Risk.status!=RiskStatus.CLOSED,Risk.score>=15)))
...
def update(id:str,b:ProjectUpdate,db:Session=Depends(get_db),user:User=Depends(require_roles(Role.ADMIN,Role.QA_MANAGER))):
 p=get_project(db,id,user.organization_id,user)
...
def delete(id:str,db:Session=Depends(get_db),user:User=Depends(require_roles(Role.ADMIN))):
 p=get_project(db,id,user.organization_id,user);audit(db,user,'DELETE','PROJECT',p.id,f'Deleted {p.code}');db.delete(p);db.commit();return Response(status_code=204)
