...
def _login_key(email: str) -> str:
    return token_hash(email.strip().lower())


def _login_state(db: Session, email: str) -> LoginSecurityState:
    key = _login_key(email)
    state = db.get(LoginSecurityState, key)
    if not state:
        state = LoginSecurityState(email_hash=key)
...
def _record_login_failure(db: Session, state: LoginSecurityState) -> None:
    ...
    started = _aware(state.window_started_at)
...
def authenticate(db: Session, email: str, password: str) -> User:
    ...
    if not user or not user.is_active or not verify_password(password, user.password_hash):
        _record_login_failure(db, state)
        raise HTTPException(status_code=401, detail="Invalid email or password")

    _clear_login_failures(db, state)
    return user
...
def sessions(payload: dict = Depends(get_current_payload), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    rows = list(db.scalars(select(AuthSession).where(AuthSession.user_id == user.id).order_by(AuthSession.created_at.desc())).all())
...
def forgot_password(body: ForgotPasswordIn, db: Session = Depends(get_db)):
    ...
    prior = list(db.scalars(select(PasswordResetToken).where(PasswordResetToken.user_id==user.id,PasswordResetToken.used_at.is_(None))).all())
    now = datetime.now(timezone.utc)
    for item in prior: item.used_at = now
    raw = token_value(); db.add(PasswordResetToken.create(user.id, raw)); db.commit()
...
def reset_password(body: ResetPasswordIn, db: Session = Depends(get_db)):
    ...
    if state:
        state.failed_count = 0; state.blocked_until = None; state.window_started_at = datetime.now(timezone.utc)
...
def request_email_verification(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    raw = token_value(); db.add(EmailVerificationToken.create(user.id,raw)); db.commit()
