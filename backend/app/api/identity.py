...
from ..identity_schemas import InvitationAccept, InvitationCreate, InvitationOut, MemberOut, MemberRoleUpdate, MemberStatusUpdate, OrganizationOut, ProjectAssignmentIn, RegisterOrganizationIn
from ..models import Project, Role, User
from ..security import create_access_token, get_current_user, hash_password, require_roles
...
def _membership_role(role: Role) -> str:
    ...
    }.get(role, MembershipRole.VIEWER.value)


def _legacy_role(role: str) -> Role:
    return {
        MembershipRole.ORGANIZATION_ADMIN.value: Role.ADMIN,
        MembershipRole.QA_MANAGER.value: Role.QA_MANAGER,
        MembershipRole.QA_ENGINEER.value: Role.QA_ENGINEER,
        MembershipRole.PROJECT_MANAGER.value: Role.VIEWER,
        MembershipRole.DOCUMENT_CONTROLLER.value: Role.VIEWER,
        MembershipRole.VIEWER.value: Role.VIEWER,
    }.get(role, Role.VIEWER)


def _ensure_org(db: Session, user: User) -> Organization:
    org = db.get(Organization, user.organization_id)
    if not org:
        org = Organization(id=user.organization_id, name="QualiCore Demo Organization" if user.organization_id == "demo-org" else "Organization", slug=_unique_slug(db, user.organization_id))
        db.add(org)
        db.flush()
    ...
    if not membership:
        db.add(OrganizationMember(organization_id=user.organization_id, user_id=user.id, role=_membership_role(user.role), status=MembershipStatus.ACTIVE.value))
        db.flush()
    return org
...
def register(body: RegisterOrganizationIn, db: Session = Depends(get_db)):
    ...
    org = Organization(name=body.organization_name.strip(), slug=_unique_slug(db, body.organization_name), country=body.country)
    db.add(org)
    db.flush()
    user = User(organization_id=org.id, email=email, full_name=body.full_name.strip(), password_hash=hash_password(body.password), role=Role.ADMIN, is_active=True)
    db.add(user)
    db.flush()
    ...
    db.commit()
    db.refresh(user)
    return {"access_token": create_access_token(user), "token_type": "bearer", "organization": {"id": org.id, "name": org.name, "slug": org.slug}, "user": {"id": user.id, "email": user.email, "full_name": user.full_name, "role": MembershipRole.ORGANIZATION_ADMIN.value}}
...
def members(db: Session = Depends(get_db), admin: User = Depends(require_roles(Role.ADMIN))):
    _ensure_org(db, admin)
    users = list(db.scalars(select(User).where(User.organization_id == admin.organization_id).order_by(User.full_name)).all())
    output = []
    for user in users:
        ...
        if not member:
            member = OrganizationMember(organization_id=admin.organization_id, user_id=user.id, role=_membership_role(user.role), status=MembershipStatus.ACTIVE.value)
            db.add(member)
            db.flush()
...
def invite(body: InvitationCreate, db: Session = Depends(get_db), admin: User = Depends(require_roles(Role.ADMIN))):
    ...
    if body.project_id:
        ...
        if not project or project.organization_id != admin.organization_id:
    ...
    if existing:
        db.delete(existing)
        db.flush()
    invitation = Invitation(organization_id=admin.organization_id, email=email, role=body.role.value, project_id=body.project_id, invited_by=admin.id)
    db.add(invitation)
    db.commit()
...
def accept_invitation(body: InvitationAccept, db: Session = Depends(get_db)):
    ...
    if db.scalar(select(User.id).where(User.email == invitation.email)):
        raise HTTPException(409, "Email already registered")
    user = User(organization_id=invitation.organization_id, email=invitation.email, full_name=body.full_name.strip(), password_hash=hash_password(body.password), role=_legacy_role(invitation.role), is_active=True)
    db.add(user)
    db.flush()
    ...
    if invitation.project_id:
        db.add(ProjectMember(organization_id=invitation.organization_id, project_id=invitation.project_id, user_id=user.id, role=invitation.role))
    invitation.accepted_at = datetime.now(timezone.utc)
    db.commit()
    db.refresh(user)
    return {"access_token": create_access_token(user), "token_type": "bearer", "user": {"id": user.id, "email": user.email, "full_name": user.full_name, "role": invitation.role}}
...
def update_role(user_id: str, body: MemberRoleUpdate, db: Session = Depends(get_db), admin: User = Depends(require_roles(Role.ADMIN))):
    if user_id == admin.id and body.role != MembershipRole.ORGANIZATION_ADMIN:
        raise HTTPException(400, "You cannot remove your own organization admin role")
    ...
    user.role = _legacy_role(body.role.value)
    db.commit()
...
def update_status(user_id: str, body: MemberStatusUpdate, db: Session = Depends(get_db), admin: User = Depends(require_roles(Role.ADMIN))):
    allowed = {MembershipStatus.ACTIVE.value, MembershipStatus.SUSPENDED.value, MembershipStatus.DISABLED.value}
    status = body.status.upper()
    if status not in allowed:
        raise HTTPException(400, "Invalid member status")
    if user_id == admin.id and status != MembershipStatus.ACTIVE.value:
        raise HTTPException(400, "You cannot disable your own account")
    user = db.get(User, user_id)
    member = db.scalar(select(OrganizationMember).where(OrganizationMember.organization_id == admin.organization_id, OrganizationMember.user_id == user_id))
    if not user or not member or user.organization_id != admin.organization_id:
        raise HTTPException(404, "User not found")
    member.status = status
    user.is_active = status == MembershipStatus.ACTIVE.value
    db.commit()
    return {"status": "ok", "member_status": status}
...
def assign_project(user_id: str, body: ProjectAssignmentIn, db: Session = Depends(get_db), admin: User = Depends(require_roles(Role.ADMIN))):
    ...
    if not project or project.organization_id != admin.organization_id:
        raise HTTPException(404, "Project not found")
    existing = db.scalar(select(ProjectMember).where(ProjectMember.project_id == body.project_id, ProjectMember.user_id == user_id))
    role = (body.role.value if body.role else _membership_role(user.role))
    if existing:
        existing.role = role
    else:
        db.add(ProjectMember(organization_id=admin.organization_id, project_id=body.project_id, user_id=user_id, role=role))
    db.commit()
    return {"status": "ok", "project_id": body.project_id, "user_id": user_id, "role": role}
...
def remove_project(user_id: str, project_id: str, db: Session = Depends(get_db), admin: User = Depends(require_roles(Role.ADMIN))):
    ...
    if not assignment:
        raise HTTPException(404, "Project assignment not found")
