...
def get_item(db: Session, item_id: str, user: User) -> PunchItem:
    item = db.scalar(select(PunchItem).where(PunchItem.id == item_id, PunchItem.organization_id == user.organization_id))
    if not item: raise HTTPException(404, "Punch item not found")
    require_project_access(db, user, item.project_id)
    return item

def label(item: PunchItem) -> str: return f"Punch {item.id[:8]}"
...
def list_items(project_id: str | None = None, status: WorkflowStatus | None = None, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    query = apply_project_scope(select(PunchItem), PunchItem, db, user)
    if project_id:
        require_project_access(db, user, project_id); query = query.where(PunchItem.project_id == project_id)
    if status: query = query.where(PunchItem.status == status)
    return db.scalars(query.order_by(PunchItem.created_at.desc())).all()

@router.get("/{item_id}", response_model=PunchOut)
def detail(item_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)): return get_item(db, item_id, user)

@router.post("", response_model=PunchOut, status_code=201)
def create(body: PunchCreate, db: Session = Depends(get_db), user: User = Depends(require_roles(Role.ADMIN, Role.QA_MANAGER, Role.QA_ENGINEER))):
    ...
    item = PunchItem(**body.model_dump(), organization_id=user.organization_id, created_by=user.id)
    db.add(item); db.flush(); audit(db, user, "CREATE", "PUNCH", item.id, f"Created {label(item)}")
    db.commit(); db.refresh(item); return item

@router.patch("/{item_id}", response_model=PunchOut)
def update(item_id: str, body: PunchUpdate, db: Session = Depends(get_db), user: User = Depends(require_roles(Role.ADMIN, Role.QA_MANAGER, Role.QA_ENGINEER))):
    item = get_item(db, item_id, user)
    if item.status == WorkflowStatus.CLOSED: raise HTTPException(400, "Closed punch item cannot be edited")
...
def close(item_id: str, db: Session = Depends(get_db), user: User = Depends(require_roles(Role.ADMIN, Role.QA_MANAGER))):
    item = get_item(db, item_id, user)
    if item.status == WorkflowStatus.CLOSED: return item
...
def delete(item_id: str, db: Session = Depends(get_db), user: User = Depends(require_roles(Role.ADMIN, Role.QA_MANAGER))):
    item = get_item(db, item_id, user); audit(db, user, "DELETE", "PUNCH", item.id, f"Deleted {label(item)}")
