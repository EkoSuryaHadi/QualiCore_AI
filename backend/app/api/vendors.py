...
def get_item(db: Session, vendor_id: str, user: User) -> VendorQuality:
    item = db.scalar(select(VendorQuality).where(VendorQuality.id == vendor_id, VendorQuality.organization_id == user.organization_id))
    if not item: raise HTTPException(404, "Vendor quality record not found")
    require_project_access(db, user, item.project_id)
    return item


def calculate_overall(item: VendorQuality) -> float:
    score = item.quality_score * .35 + item.delivery_score * .20 + item.documentation_score * .20 + item.inspection_score * .25
    score -= min(item.ncr_count * 2.0, 20.0)
    return round(max(0.0, min(100.0, score)), 1)

@router.get("", response_model=list[VendorOut])
def list_items(project_id: str | None = None, status: VendorStatus | None = None, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    query = apply_project_scope(select(VendorQuality), VendorQuality, db, user)
    if project_id:
        require_project_access(db, user, project_id); query = query.where(VendorQuality.project_id == project_id)
    if status: query = query.where(VendorQuality.status == status)
    return db.scalars(query.order_by(VendorQuality.overall_score.asc(), VendorQuality.vendor_name.asc())).all()

@router.get("/{vendor_id}", response_model=VendorOut)
def detail(vendor_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)): return get_item(db, vendor_id, user)

@router.post("", response_model=VendorOut, status_code=201)
def create(body: VendorCreate, db: Session = Depends(get_db), user: User = Depends(require_roles(Role.ADMIN, Role.QA_MANAGER, Role.QA_ENGINEER))):
    ...
    item = VendorQuality(**body.model_dump(), organization_id=user.organization_id, created_by=user.id)
    item.overall_score = calculate_overall(item); db.add(item)
    try: db.flush()
    except IntegrityError:
        db.rollback(); raise HTTPException(409, "Vendor code already exists for this project")
...
def update(vendor_id: str, body: VendorUpdate, db: Session = Depends(get_db), user: User = Depends(require_roles(Role.ADMIN, Role.QA_MANAGER, Role.QA_ENGINEER))):
    item = get_item(db, vendor_id, user)
