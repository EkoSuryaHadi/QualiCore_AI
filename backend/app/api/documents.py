...
from ..access_control import apply_project_scope, require_membership_roles, require_project_access
...
def get_item(db,id,user):
    item=db.scalar(select(Document).where(Document.id==id,Document.organization_id==user.organization_id))
    if not item: raise HTTPException(404,"Document not found")
    require_project_access(db,user,item.project_id); return item
...
def list_docs(project_id:str|None=None,status:DocumentStatus|None=None,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    q=apply_project_scope(select(Document),Document,db,user)
    if project_id:
        require_project_access(db,user,project_id); q=q.where(Document.project_id==project_id)
...
editor_roles=(MembershipRole.ORGANIZATION_ADMIN,MembershipRole.QA_MANAGER,MembershipRole.QA_ENGINEER,MembershipRole.DOCUMENT_CONTROLLER)
reviewer_roles=(MembershipRole.ORGANIZATION_ADMIN,MembershipRole.QA_MANAGER)

@router.post("",response_model=DocumentOut,status_code=201)
def create(b:DocumentCreate,db:Session=Depends(get_db),user:User=Depends(require_membership_roles(*editor_roles))):
    require_project_access(db,user,b.project_id)
...
def update(id:str,b:DocumentUpdate,db:Session=Depends(get_db),user:User=Depends(require_membership_roles(*editor_roles))):
    x=get_item(db,id,user)
...
def submit(id:str,b:WorkflowAction,db:Session=Depends(get_db),user:User=Depends(require_membership_roles(*editor_roles))):
    x=get_item(db,id,user)
...
def approve(id:str,b:WorkflowAction,db:Session=Depends(get_db),user:User=Depends(require_membership_roles(*reviewer_roles))):
    x=get_item(db,id,user)
...
def reject(id:str,b:WorkflowAction,db:Session=Depends(get_db),user:User=Depends(require_membership_roles(*reviewer_roles))):
    x=get_item(db,id,user)
...
def history(id:str,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    get_item(db,id,user);return db.scalars(select(WorkflowEvent).where(WorkflowEvent.organization_id==user.organization_id,WorkflowEvent.entity_type==WorkflowEntity.DOCUMENT,WorkflowEvent.entity_id==id).order_by(WorkflowEvent.created_at)).all()
