...
export default function VendorsPage(){
 ...
 const badge=(s:string)=>s==="APPROVED"?"workflow-approved":s==="WATCHLIST"?"workflow-in_review":"workflow-rejected";
 return <AuthGuard><Shell>
  ...
  {!visible.length?<div className="empty-state">No vendor quality records found.</div>:<div className="table-wrap"><table className="data-table"><thead><tr><th>Vendor</th><th>Category</th><th>Status</th><th>Score</th><th>NCR</th><th></th></tr></thead><tbody>{visible.map(v=><tr key={v.id}><td><strong>{v.vendor_code}</strong><div className="muted small">{v.vendor_name}</div></td><td>{v.category}</td><td><span className={`workflow ${badge(v.status)}`}>{v.status}</span></td><td><span className={`risk-score ${v.overall_score>=85?"risk-score-low":v.overall_score>=70?"risk-score-medium":"risk-score-high"}`}>{v.overall_score.toFixed(1)}</span></td><td>{v.ncr_count}</td><td><Link className="text-link" href={`/vendors/${v.id}`}>View →</Link></td></tr>)}</tbody></table></div>}</div>
 </Shell></AuthGuard>
}
